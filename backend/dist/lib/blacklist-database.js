...
exports.blacklistDatabase = void 0;
class BlacklistDatabaseService {
    ...
    async checkBlacklist(value, type) {
        const matches = [];
        if (type === "address") {
    ...
    async getAllEntries(query = {}) {
        ...
        if (query.search) {
            ...
            entries = entries.filter((entry) => entry.value.toLowerCase().includes(searchLower) ||
    ...
    async searchSimilar(value, type) {
        const entries = Array.from(this.entries.values());
        const similar = [];
        entries.forEach((entry) => {
...
exports.blacklistDatabase = new BlacklistDatabaseService();
//# sourceMappingURL=blacklist-database.js.map